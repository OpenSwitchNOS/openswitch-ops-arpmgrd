...
struct neighbor_data {
    ...
    unsigned int revalidate_del; /* this flag is to avoid db update for nbr entry*/
                ...
                */
                ...
                if((!(strcmp(port_cache->admin, PORT_CONFIG_ADMIN_DOWN)) &&
                    ...
                    VLOG_INFO("Port %s either came up or ip4 resotred,<%d>. cur state: %s %s",
                        ovs_port->name,no_ipv4, port_cache->admin, port_cache->ipv4_address);
                    port_cache->port_up_reinstal.name = ovs_port->name;
                ...
                if (ovs_port->admin && (strcmp(port_cache->admin, ovs_port->admin))) {
                     VLOG_INFO("port%s admin statemodified from %s to %s",ovs_port->name, port_cache->admin, ovs_port->admin);
    ...
    VLOG_INFO("Sync with kernel was called: nbr_alloc_cnt %d, multi_part_res %d, no_n_res %d, nl_dump_res_size %d",
        ...
        if(reinstal_nh_port_up){
            ...
            SHASH_FOR_EACH_SAFE (sh_nbr, sh_nbr_next, &all_neighbors) {
                ...
                bool no_ipv4 = pport_data->port_up_reinstal.ipv4;
                if (nbr_cache->device && pport_data->port_up_reinstal.name
                    && !strcmp(nbr_cache->device, pport_data->port_up_reinstal.name)) {
                    if(nbr_cache->routes_nh) {
                        VLOG_INFO("pinging restore nh after port %s up ", nbr_cache->device);
                        if(!strcmp(nbr_cache->network_family,
                            OVSREC_NEIGHBOR_ADDRESS_FAMILY_IPV4)) {
                            ping4(nbr_cache->ip_address);
                            if(no_ipv4)
                                continue;
                        }
                        if(!strcmp(nbr_cache->network_family, OVSREC_NEIGHBOR_ADDRESS_FAMILY_IPV6)) {
                            ping6(nbr_cache->ip_address);
                        }
                    }
                }
            }
