 ...
 * prev_ns: Used to store the switch namespace.
 * set_ns : Used to store the VRF_name.
 * */
static void
netlink_socket_open(const char *vrf_name, int *sock, int protocol, int group, bool kernel_dump)
{
    ...
    if (vrf_name && (strcmp(vrf_name , DEFAULT_VRF_NAME)) != 0)
    {
       /* non default vrf. We need to open socket by entering corresponding namespace
        ...
        if (fd_from_ns == -1) {
            VLOG_ERR("Unable to open fd for namepsace %s line number %d %s",
                      SWITCH_NAMESPACE,__LINE__, strerror(errno));
            goto label;
        }
                      ...
                      const struct vrf *vrf, struct neighbor_data **cache_nbr)
{
    ...
    if (rta->rta_type == NDA_DST) {
        ...
        get_vrf_ns_from_name (idl, vrf->name, buff);
...
{
    const struct ovsrec_neighbor *ovs_nbr;
    char dev[IF_NAMESIZE], buff[UUID_LEN] = {0};
    ...
    while (NLMSG_OK(nlh, msglen)) {
        ...
        if (!(ndm->ndm_state & NUD_NOARP)) {
            ...
            if (ndm->ndm_family != AF_INET && ndm->ndm_family != AF_INET6) {
             ...
             get_vrf_ns_from_name(idl, vrf->name, buff);
             nl_if_indextoname(ndm->ndm_ifindex, ifname, buff);
            ...
            local_vrf = find_port_vrf_in_cache(ifname);
            /*
            ...
            for (; RTA_OK(rta, rtalen); rta = RTA_NEXT(rta, rtalen)) {
                if (nlh->nlmsg_type == RTM_NEWNEIGH) {
                    update_neighbor_cache(sock, ndm, rta, local_vrf, &cache_nbr);
                } else if (nlh->nlmsg_type == RTM_DELNEIGH) {
                    /* delete cache and ovsdb */
                    del_neighbor(ndm, rta, local_vrf);
                }
            }
...
arpmgrd_vrf_add(const struct ovsrec_vrf *vrf_row)
{
    ...
    char buff[UUID_LEN]={0};
    struct vrf *vrf;
    get_vrf_ns_from_name (idl, vrf_row->name, buff);
    ...
    if (strcmp(vrf->name, DEFAULT_VRF_NAME) != 0)
    {
        ...
        netlink_socket_open(buff, &(vrf->nl_sock),  NETLINK_ROUTE, RTMGRP_NEIGH, kernel_dump);
    }
    else
    {
    ...
    shash_init(&vrf->ports);
    shash_add_once(&all_vrfs, vrf->name, vrf);
...
arpmgrd_add_del_vrf(struct ovsdb_idl *idl)
{
    ...
    SHASH_FOR_EACH_SAFE (node, next, &all_vrfs) {
        ...
        get_vrf_ns_from_name (idl, vrf->name, buff);
        vrf->cfg = shash_find_data(&new_vrfs, (const char*)buff);
        if (!vrf->cfg) {
    ...
    OVSREC_VRF_FOR_EACH (vrf_row, idl) {
        ...
        if (!vrf) {
            if (!strcmp(vrf_row->name, DEFAULT_VRF_NAME)
                || !strcmp(smap_get(&vrf_row->status, VRF_STATUS_KEY),
                                    VRF_STATUS_VALUE)) {
...
arpmgrd_run(void)
{
    ...
    VLOG_INFO("Sync with kernel was called: nbr_alloc_cnt %d, multi_part_res %d, no_n_res %d, nl_dump_res_size %d",
        ...
        if(sync_state == SYNC_REQUESTED || sync_state == SYNC_FAILED) {
            ...
            if(sync_mode == SYNC_WITH_CACHE_RESET) {
                ...
                if(sync_state != SYNC_FAILED) {
                   ...
                   netlink_socket_open(DEFAULT_VRF_NAME, &nl_neighbor_sock, NETLINK_ROUTE, 0, kernel_dump);
                }
            }
        ...
        if (!nl_neighbor_sock) {
            ...
            netlink_socket_open(DEFAULT_VRF_NAME, &nl_neighbor_sock, NETLINK_ROUTE, RTMGRP_NEIGH, kernel_dump);
        }
        /*
         * If previous transaction status was incomplete
         * check status again. Else destroy previous
