...
def arp_manager_configure_and_setup(sw1, hs1, hs2, step):
    ...
    sw1("/usr/bin/ovs-vsctl set interface "
        "{sw1p1} user_config:admin=up".format(**locals()),
        shell='bash')
    # Configure interface 2
    sw1("/usr/bin/ovs-vsctl set interface "
        "{sw1p2} user_config:admin=up".format(**locals()),
        shell='bash')
    # Configure interface 3
    sw1("/usr/bin/ovs-vsctl set interface "
        "{sw1p3} user_config:admin=up".format(**locals()),
        shell='bash')
...
def arp_manager_ovsdb_update(sw1, hs1, hs2, step):
    ...
    assert port == "{sw1p1}".format(**locals())
    ...
    assert mac == mac2
    port = words[port_index]
    assert port == "{sw1p2}".format(**locals())
    state = words[state_index]
    assert state == 'reachable'
